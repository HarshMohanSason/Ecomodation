...
class _AddListingState extends State<AddListing> with TickerProviderStateMixin {
  ...
  double fontSize(
      ...
      double
          baseFontSize) //Handle the FontSizes according to the respective screen Sizes
  {
    //Using the size of text on the Emulator as the baseFontSize.
    final fontSize = baseFontSize *
        (screenHeight /
            932); //Note, we divide by 932 because it is the original base height of the logical pixels of the emulator screen
  ...
  {
    setState(() {
      AddListing.allImages
          .add(image!.path); //adding the angle to each individual image
  ...
  Future<void> addImageFromGallery() async {
    ...
    try {
      var image =
          await picker.pickMultiImage(); //get the image from the gallery
      setState(() {
